  ...
  if (isset($_POST['update'])) {
    ...
    $query = mysqli_query($conn, "UPDATE user SET nama='$nama',no_hp='$nohp',password='$password' WHERE email='$email'");
    $_SESSION['message'] = "Berhasil update profile";
    setcookie('nav', $nav, strtotime('+3 days'), '/');
    $_SESSION['nama'] = $nama;
    header("location: Profile.php");
  }
  $user_id = $_SESSION['id'];
  $query = mysqli_query($conn, "SELECT * FROM user WHERE id = '$user_id' LIMIT 1");
              ...
              <div class="card-boddy pt-0">
                <form action="" method="POST">
                  <div class="form-group mb-3">
                    ...
                    <input required type="email" class="form-control" id="email" name="email" placeholder="email@example.com" readonly value="<?= $select['email'] ?>">
                  </div>
                  <div class="form-group mb-3">
                    <label for="nama" class="form-label">Nama</label>
                    <input required type="text" class="form-control" id="nama" name="nama" placeholder="Nama" value="<?= $select['nama'] ?>">
                  </div>
                  <div class="form-group mb-3">
                    <label for="" class="form-label">No. Handphone</label>
                    <input required type="number" class="form-control" id="nomorhp" name="nomorhp" placeholder="No. Handphone" value="<?= $select['no_hp'] ?>">
                  </div>
                  <hr>
                  <div class="form-group mb-3">
                  ...
                  <div class="form-group mb-3">
                    ...
                    <input required type="password" class="form-control" id="passwordconfirm" name="passwordconfirm" placeholder="Confirm Password">
                  </div>
                  <select class="form-select" aria-label="Default select example" name="nav">
                    <option selected value="<?= $nav ?>"><?= $nav ?></option>
                  ...
                  <div class="text-center pt-2">
                    ...
                    <a href="Login.php" class="text-secondary btn btn-warning">Cancel</a>
                  </div>
                </form>
              </div>
            </div>
